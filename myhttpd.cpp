...
"                                                               \n"
"Where 1024 < port < 65536. Default port is 5555 			    \n"
"                                                               \n"
"                                                               \n";
...
processTimeRequest( int fd )
{
  ...
  contentType=strstr(name,"Content-Type:");
  if(contentType){
  	contentType=strdup(strstr(name,"Content-Type:")+13);
  	char* ct=strstr(contentType,"\r\n");
  	if(ct)
  		*ct='\0';
  }

  contentLength=strstr(name,"Content-Length:");
  if(contentLength){
  	contentLength=strdup(strstr(name,"Content-Length:")+15);
  	char* cl=strstr(contentLength,"\r\n");
  	if(cl)
 	 	*cl='\0';
  }   
...
if(mode){
  ...
  if(expandPath(filePath,cwd)==NULL){
	...
	}else getRequestEndTime(requestTime,docPath);
	...
	FILE* readfile=fopen(filePath,"rb");
  	...
  	if(readfile==NULL){
  		...
  		if(strstr(docPath, "/cgi-bin")){
			...
			if(!strcmp(method,"POST")&&contentType){
				setenv("CONTENT_TYPE",contentType,1);				
			}else putenv((char*)"CONTENT_TYPE=");
			
			if(!strcmp(method,"POST")&&contentLength){
				setenv("CONTENT_LENGTH",contentLength,1);				
			}else putenv((char*)"CONTENT_LENGTH=");
			...
			if(mode&&!strcmp(mode,"-f")&&(!strcmp(docPath,"/stats")||!strcmp(docPath,"/stats.html"))){
				char buffer[2014];
...
void
exeCgi(int fd,int argv,char* queryString,char* exefile,char* protocol){
		...
		if(getenv("CONTENT_LENGTH")!=NULL){	
			...
			buffer[length]='\0';
			write(serverTOcgi[1],buffer,length);
